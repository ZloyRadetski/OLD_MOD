package net.portalmod.common.sorted.antline;

import java.util.function.Supplier;
import net.minecraft.block.Block;
import net.minecraft.client.Minecraft;
import net.minecraft.nbt.CompoundNBT;
import net.minecraft.network.PacketBuffer;
import net.minecraft.util.math.BlockPos;
import net.minecraftforge.api.distmarker.Dist;
import net.minecraftforge.fml.DistExecutor;
import net.minecraftforge.fml.network.NetworkEvent;
import net.portalmod.core.init.BlockInit;
import net.portalmod.core.packet.AbstractPacket;

public class SAntlineUpdatePacket implements AbstractPacket<SAntlineUpdatePacket> {
   private BlockPos pos;
   private CompoundNBT nbt;

   public SAntlineUpdatePacket() {
   }

   public SAntlineUpdatePacket(BlockPos pos, CompoundNBT nbt) {
      this.pos = pos;
      this.nbt = nbt;
   }

   public void encode(PacketBuffer buffer) {
      buffer.func_179255_a(this.pos).func_150786_a(this.nbt);
   }

   public SAntlineUpdatePacket decode(PacketBuffer buffer) {
      return new SAntlineUpdatePacket(buffer.func_179259_c(), buffer.func_150793_b());
   }

   public boolean handle(Supplier<NetworkEvent.Context> context) {
      ((NetworkEvent.Context)context.get()).enqueueWork(() -> DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> {
               AntlineTileEntity blockEntity = (AntlineTileEntity)Minecraft.func_71410_x().field_71441_e.func_175625_s(this.pos);
               if (blockEntity != null) {
                  blockEntity.load(this.nbt, false);
                  Minecraft.func_71410_x().field_71441_e.func_184138_a(this.pos, ((Block)BlockInit.ANTLINE.get()).func_176223_P(), ((Block)BlockInit.ANTLINE.get()).func_176223_P(), 0);
                  blockEntity.requestModelDataUpdate();
               }
            }));
      ((NetworkEvent.Context)context.get()).setPacketHandled(true);
      return true;
   }
}
